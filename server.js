...
// ---------- Socket.IO логика (с доступом к данным для админки) ----------
const usersQueue = [];
const activeRooms = new Map();
const sessionMap = new Map();
let onlineCount = 0;

// Экспортируем для админки
const getStats = () => ({
  online: onlineCount,
  ...
  avgDuration: sessions.reduce((acc, s) => {
    ...
    return acc;
  }, 0) / (sessions.filter(s => s.endTime).length || 1) / 60000, // в минутах
...
app.get('/admin', (req, res) => {
  ...
  <div class="container">
    ...
    <!-- Фильтры и поиск -->
    <div class="section">
      <h2>🔍 Сессии (фильтр + поиск)</h2>
      <form method="GET" action="/admin" class="filters">
        <label>Поиск: <input type="text" name="search" value="${search}" placeholder="ID, socketId, пол..."></label>
      ...
      <table>
        <tr><th>ID</th><th>Socket</th><th>Пол</th><th>Возраст</th><th>Начало</th><th>Конец</th><th>Сообщений</th><th>Действие</th></tr>
        ${pagedSessions.map(s => `
          <tr>
            <td>${s.id}</td>
        ...
        `).join('') || '<tr><td colspan="8">Нет сессий</td></tr>'}
      </table>
    ...
    <!-- Последние сообщения (без фильтра) -->
    <div class="section">
...
app.get('/admin/session/:id', (req, res) => {
  ...
  <p>Socket: ${session.socketId}</p>
  <p>Пол: ${session.gender}, Возраст: ${session.age}</p>
  <p>Начало: ${new Date(session.startTime).toLocaleString()}</p>
  <p>Конец: ${session.endTime ? new Date(session.endTime).toLocaleString() : '—'}</p>
  <p>Сообщений: ${session.messagesCount || 0}</p>
  <h3>Сообщения</h3>
...
app.get('/admin/export/sessions', (req, res) => {
  let csv = 'ID, Socket, Пол, Возраст, Начало, Конец, Сообщений\n';
  sessions.forEach(s => {
    csv += `${s.id},${s.socketId},${s.gender},${s.age},${new Date(s.startTime).toISOString()},${s.endTime ? new Date(s.endTime).toISOString() : ''},${s.messagesCount || 0}\n`;
  });
