...
// ---------- Админ-панель (полная, с новыми блоками) ----------
  ...
  <div class="container">
    ...
    <div class="section">
      ...
      <form method="POST" action="/admin/blacklist/add" style="display:flex; gap:8px; margin-bottom:10px;">
        ...
        <button type="submit" class="ban-btn">Добавить в чёрный список</button>
      </form>
      <ul>
        ${blacklist.map(ip => `
        ...
        ${pagedSessions.map(s => `
          <tr>
            ...
            <td>
              ...
              ${s.ip ? `<form method="POST" action="/admin/blacklist/add" style="display:inline;" onsubmit="return confirm('Заблокировать IP ${s.ip}?');"><input type="hidden" name="ip" value="${s.ip}"><button type="submit" class="ban-btn">Заблокировать IP</button></form>` : ''}
            </td>
          </tr>
...
// Экспорт CSV (с расширенными полями)
...
io.on('connection', (socket) => {
  ...
  // Остальные обработчики (find, message, next, disconnect) – без изменений, но с обновлением lastActivity
  socket.on('find', (data) => {
    lastActivity.set(socket.id, Date.now());
    // ... (код find из предыдущей версии)
  });

  socket.on('message', (data) => {
    lastActivity.set(socket.id, Date.now());
    // ... (код message из предыдущей версии)
  });

  socket.on('next', () => {
    lastActivity.set(socket.id, Date.now());
    // ... (код next из предыдущей версии)
  });

  socket.on('report', (data) => {
    // Ожидаем { targetSocket, reason }
    const report = {
      id: nextReportId++,
  ...
  socket.on('disconnect', () => {
    ...
    onlineCount--;
    io.emit('onlineCount', onlineCount);
    // ... (остальной код disconnect из предыдущей версии, с обновлением endTime)
  });
});
